...
function IncomeChart() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            if (userinfoString) {
                ...
                const { data } = await axios.get(`/api/host/getmonthlyincome/${userinfo._id}`)
                if (data) {
    ...
    useEffect(() => {
        ...
        if (window.myhosticomeBar && typeof window.myhosticomeBar.destroy === 'function') {
            window.myhosticomeBar.destroy();
        }

        const ctx = canvasRef.current.getContext('2d');
        window.myhosticomeBar = new Chart(ctx, config);
    }, [incomeData]);
