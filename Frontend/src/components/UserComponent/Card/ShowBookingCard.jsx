...
function ShowBookingCard() {
    ...
    const handleCancel = async (bookingId) => {
        try {
          const {data} = await axios.post(`/api/user/cancelbook/${bookingId}`,{});
           if(data){
                        ...
                        >
                            ...
                            <div className="sm:flex sm:justify-between sm:gap-4">
                                ...
                                <div className="hidden sm:block sm:shrink-0">
                                    <img
                                        alt="Paul Clapton"
                                        src={data?.spaceID.PicData.ImageUrl[0]}
                            ...
                            <div className='flex gap-9'>
                                <label
                                    className={`inline-block py-1 px-2 rounded text-sm font-semibold ${data.bookingApproved ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                                        }`}
                                >
                                    Status: {data.bookingApproved ? 'Approved' : 'Not Approved'}
                                </label>
                                <button
